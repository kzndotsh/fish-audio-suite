...
Need `uv sync --all-packages --extra cli --group dev` first if `.venv` is
missing or lock/deps changed.

## Path → commands

Union rows. Fastest first: format/lint → types → tests → nix.

| Dirty paths | Run |
| --- | --- |
| `packages/**/*.py`, `pyproject.toml` | `uv run ruff format packages`, `uv run ruff check packages`, `uv run basedpyright` |
...
basedpyright, and full pytest after `uv sync --frozen --all-packages --extra cli --group dev`.
