...
| `barge.py` | VAD record + barge. Bleed: `effective_bleed_s` at arm (AEC short vs `FISH_VOICE_BLEED_DELAY` 0.9). While far-end is playing, barge RMS is `BARGE_RMS` × 2.2 so echo/hiss cannot cut TTS. Listen `FISH_VOICE_MIN_RMS` is a floor that only rises. Start requires VAD **and** full min RMS; a 4x peak in the pre-pad needs 10 start hits. Impulse reject if peak >= 4x floor and voiced hits <= 24. Hold ratio is post-start only. |
