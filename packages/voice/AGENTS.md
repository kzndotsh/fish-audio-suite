...
| `barge.py` | VAD record + barge (`FISH_VOICE_BLEED_DELAY` default 0.9, read at construct/call). RMS + webrtcvad; `FISH_VOICE_SPEECH_FRAMES` default 8. Kit BoH/gzip is the ASR post-filter, not a second VAD. |
| `cli.py` | Mic → Fish ASR → LLM → `speak_isolated` (full reply, then TTS). OpenRouter `:nitro` lives here. No default LLM model. Omit ASR `language` unless `FISH_ASR_LANGUAGE`. `FISH_BASE` from env. One sampled W3C trace id per turn on ASR REST + live TTS. ASR/TTS 401/402/403 exit 2. |
...
`--smoke` uses `FileSink`; exits 2 if `FISH_API_KEY` or `FISH_VOICE_ID` is missing. The duplex loop does not call `speak_deltas`.
